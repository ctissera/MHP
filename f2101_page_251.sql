...
--   Date and Time:   10:25 Martes Diciembre 20, 2016
...
'No hay ayuda disponible para esta página.'
 ...
 ,p_last_upd_yyyymmddhh24miss => '20161214133011'
  );
null;
...
' and com_pkg.obtn_comdoc_no_fmt3 ( p_segorg_id => COMPDO.SEGORG_';
...
s:=s||'nd    comobe.ESTADO = ''HAB'''||unistr('\000a')||
'and   (select comodt.SITUACION from COM_OBJETOS_DOT comodt where comodt.ID = comobd.COMODT_ID) = ''VIG'''||unistr('\000a')||
''||unistr('\000a')||
'and    comobd.ID = COMPRV_PKG.COMOBD_PRESENTADO_SN2(p_segorg_id => :P1_SEGORG_ID'||unistr('\000a')||
'                                        ,p_comodt_id => comobd.COMODT_ID'||unistr('\000a')||
'                                        ,p_comprv_id => comobj.COMPRV_ID --:P251_ID'||unistr('\000a')||
'                              ';

s:=s||'          ,p_comobj_id => comobj.ID)'||unistr('\000a')||
''||unistr('\000a')||
'order by comobd.COMOBJ_CODIGO, comobd.COMODT_NOMBRE                   ';
...
'order by comode.COMO';

s:=s||'BJ_CODIGO, comode.NOMBRE';
