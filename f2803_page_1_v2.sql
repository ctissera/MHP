...
--   Date and Time:   11:56 Martes Noviembre 1, 2016
...
'No hay ayuda disponible para esta página.'
 ...
 ,p_last_upd_yyyymmddhh24miss => '20161028124439'
  );
null;
...
s:=s||'nd comcon_pkg.comcon_comprv_estado(p_comcon_id => comcon.id, p_comprv_id => comprv.id) = ''ABIERTO'''||unistr('\000a')||
'  and sysdate >= comcon.fecha_inicio_propuesta and sysdate <= comcon.fecha_fin_propuesta'||unistr('\000a')||
'  and decode(comcoe.estado,''NUE'','''',''PEN'','''',comcon_pkg.comcon_comprv_ctzcnt(p_comcon_id => comcon.id, p_comprv_id => comprv.id)) > 0)  value, ''f?p=COMCON:310:&SESSION.::NO:RIR,310:G_APP_ANTERIOR,G_APP_ANTERIOR_P';

s:=s||'AG,P310_COMPRV_ID,P310_FILTRO_PREDEFINIDO_SEL:&APP_ID.,&APP_PAGE_ID.,&P1_COMPRV_ID.,PAC''  url,100 pct'||unistr('\000a')||
'from dual';
...
s:=s||'m.comcon_id = comcon.id'||unistr('\000a')||
'  and comcon.comctt_id = comctt.id'||unistr('\000a')||
...
'  and comoce.estado = ''APR'') value, ''f?p=COMCON:410:&SESSION.::NO:RIR,410:G_APP_ANTERIOR';

s:=s||',G_APP_ANTERIOR_PAG,P410_COMPRV_ID,P410_FILTRO_PREDEFINIDO_SEL:&APP_ID.,&APP_PAGE_ID.,&P1_COMPRV_ID.,OPDE'' url,100 pct'||unistr('\000a')||
...
'  and comocm.segorg_id = :P1_SEGORG_ID'||unistr('\000a')||
'  and comocm.comprv_id = :P1_COMPRV_ID'||unistr('\000a')||
'  and COMOCM_PKG.COMCET_PENDIENTE_SN(p_comcet_id =';

s:=s||'> comcet.id) = ''SI'''||unistr('\000a')||
...
'  and comocm.segorg_id = :P1_SEGORG_ID'||unistr('\000a')||
'  and comocm.comprv_id = :P1_COMPRV_ID'||unistr('\000a')||
'  and COMOCM_PKG.COMCET_AUTORIZADA_SN(p_comcet';

s:=s||'_id => comcet.id) = ''NO'''||unistr('\000a')||
...
s:=s||' and comocm.comcon_id = comcon.id'||unistr('\000a')||
'  and comcon.comctt_id = comctt.id'||unistr('\000a')||
'  and comocm.comprv_id = comprv.id'||unistr('\000a')||
'  and comocm.comped_id = comped.id'||unistr('\000a')||
'  and comocm.comeco_id = comeco.id'||unistr('\000a')||
...
s:=s||'  and decode(comoce.estado,''NUE'','''',''ANU'','''',''RCH'','''',''APR'','''',COM_PKG.COMREP_REPLICAR_SN(p_comocm_id => comocm.id,p_origen => ''PRV''))= ''SI'') value, ''f?p=COMCON:410:&SESSION.::NO:RIR,410:G_APP_ANTERIOR,G_APP_ANTERIOR_PAG,P410_COMPRV_ID,P410_FILTRO_PREDEFINIDO_SEL:&APP_ID.,&APP_PAGE_ID.,&P1_COMPRV_ID.,OCPR'' url,100 pct'||unistr('\000a')||
'from dual'||unistr('\000a')||
'';
...
'  and comocm.segorg_id = :P1_SEGORG_ID'||unistr('\000a')||
'  and comocm.comprv_id = :P1_COMPRV_ID'||unistr('\000a')||
'  and COMOCM_PKG.COMCET_VENCIDA_SN(p_comcet_id => co';

s:=s||'mcet.id) = ''SI'''||unistr('\000a')||
...
s:=s||'o_fmt3 ( p_segorg_id => COMPDT.SEGORG_ID'||unistr('\000a')||
...
s:=s||'                   , P_COMPRT_ID => comptt.comprt_id) in (''NO PRESENTADO'',''VENCIDO'')'||unistr('\000a')||
...
s:=s||'.comprv_id = :P1_COMPRV_ID'||unistr('\000a')||
'  and com_pkg.obtn_comdoc_no_fmt5 ( p_segorg_id => COMPDT.SEGORG_ID'||unistr('\000a')||
...
s:=s||'DO => COMPDT.COMPDE_ESTADO) in (''NO PRESENTADO'',''VENCIDO'') '||unistr('\000a')||
'union all'||unistr('\000a')||
...
'and    comode.ESTADO <> ''PRESENTADO''))  value, ''f?p=COMCON:510:&SESSION.::NO:RIR,';

s:=s||'510:G_APP_ANTERIOR,G_APP_ANTERIOR_PAG,P510_COMPRV_ID:&APP_ID.,&APP_PAGE_ID.,&P1_COMPRV_ID.'' url, 100 pct'||unistr('\000a')||
'from dual';
...
s:=s||'and (comcoe.estado = ''ACT'' '||unistr('\000a')||
'                and sysdate >= comcon.fecha_inicio_propuesta '||unistr('\000a')||
'                and sysdate <= comcon.fecha_fin_propuesta )'||unistr('\000a')||
'  and decode(comcoe.estado,''NUE'','''',''PEN'','''',comcon_pkg.comcon_comprv_ctzcnt(p_comcon_id => comcon.id, p_comprv_id => comprv.id)) = 0) value, ''f?p=COMCON:310:&SESSION.::NO:RIR,310:G_APP_ANTERIOR,G_APP_ANTERIOR_PAG,P310_COMPRV_ID,P310_FILTRO_PREDEFINI';

s:=s||'DO_SEL:&APP_ID.,&APP_PAGE_ID.,&P1_COMPRV_ID.,PANC'' url,100 pct'||unistr('\000a')||
'from dual'||unistr('\000a')||
'';
