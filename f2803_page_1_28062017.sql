...
--   Date and Time:   13:32 Miércoles Junio 28, 2017
...
'No hay ayuda disponible para esta página.'
 ...
 ,p_last_upd_yyyymmddhh24miss => '20170607145135'
  );
null;
...
'  and comcon.comctt_id = comct';

s:=s||'t.id'||unistr('\000a')||
...
s:=s||'c(sysdate) > trunc( comocm_pkg.OBTN_COMOCM_FECHA_GENERACION(comocm.comcon_id, comocm.comprv_id) )'||unistr('\000a')||
'  and comocm.comoce_estado = ''APR'') value, ''f?p=COMCON:410:&SESSION.::NO:RIR,410:G_APP_ANTERIOR,G_APP_ANTERIOR_PAG,P410_COMPRV_ID,P410_FILTRO_PREDEFINIDO_SEL:&APP_ID.,&APP_PAGE_ID.,&P1_COMPRV_ID.,OPDE'' url,100 pct'||unistr('\000a')||
...
'  and comptt.obligatorio_sn = ''SI'''||unistr('\000a')||
'  and comptt.comprt_id = (select comprt_id from com_proveedores comprv where comprv.id = :P1_COM';
...
s:=s||'COMPDT.COMPDE_ESTADO) in (''NO PRESENTADO'',''VENCIDO'')'||unistr('\000a')||
''||unistr('\000a')||
...
'  and comtdr.obligatorio_sn = ''SI'''||unistr('\000a')||
'  and compdt.compde_estado = ''HAB'''||unistr('\000a')||
'  and ';

s:=s||'comprr.comprv_id = :P1_COMPRV_ID'||unistr('\000a')||
'  and com_pkg.obtn_comdoc_no_fmt2 ( p_segorg_id => COMPDT.SEGORG_ID'||unistr('\000a')||
...
s:=s||'E_ESTADO => COMPDT.COMPDE_ESTADO) in (''NO PRESENTADO'',''VENCIDO'') '||unistr('\000a')||
''||unistr('\000a')||
'union all'||unistr('\000a')||
''||unistr('\000a')||
'select comobd.comodt_id'||unistr('\000a')||
...
s:=s||'er by comobdi.comobj_id'||unistr('\000a')||
'                                        ,comobdi.comodt_id'||unistr('\000a')||
...
'       and comobdi.fecha_vencimiento ';

s:=s||'is not null'||unistr('\000a')||
'       and comobji.comprv_id      = :P1_COMPRV_ID'||unistr('\000a')||
'     GROUP BY COMOBDI.COMOBJ_ID, comobdi.comodt_id, fecha_vencimiento'||unistr('\000a')||
'       ) comobd'||unistr('\000a')||
'       '||unistr('\000a')||
...
s:=s||'_DOC_ESTADO_V comode'||unistr('\000a')||
...
'*/'||unistr('\000a')||
''||unistr('\000a')||
'))  value, ''f?p=COMCON:510:&SESSION.::NO:RIR,510:G_APP_ANTERIOR,G_APP_ANTERIOR_PAG,P510_COMPRV_ID:&APP_ID.,&APP_PAGE_ID.,&P1_COMPRV_ID.'' url, 100 pct'||unistr('\000a')||
'from dual';
...
',(SELECT TO_CHAR(COUNT(1)) CANTIDAD '||unistr('\000a')||
'from (SELECT  compra1.ID'||unistr('\000a')||
...
'        WHERE COMPRA1.SITUACION NOT IN';

s:=s||' (''NUE'',''ANU'')'||unistr('\000a')||
'        AND COMPRA1.COMPRV_ID = :P1_COMPRV_ID) COMPRA'||unistr('\000a')||
'WHERE RANK = 1'||unistr('\000a')||
'and comprv_pkg.compra_debe_sn(p_compra_id => compra.id) = ''SI'' ) value'||unistr('\000a')||
', ''f?p=ADMCOM:PRVDET:&SESSION.::NO:251:P251_ID,P251_APP_ANTERIOR,P251_PAG_ANTERIOR,P251_ENFOCAR_AUT:&P1_COMPRV_ID.,&APP_ID.,&APP_PAGE_ID.,SI'' url'||unistr('\000a')||
', 100 PCT '||unistr('\000a')||
'from dual'||unistr('\000a')||
''||unistr('\000a')||
'/*select ''Documentación adeudada por Autoridades'' label'||unistr('\000a')||
',(select to_char(coun';
...
s:=s||'             and compra1.comprv_id = :P1_COMPRV_ID '||unistr('\000a')||
