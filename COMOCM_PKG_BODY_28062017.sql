...
--
   ...
   v_fecha_generacion    date := sysdate - 10;
   v_visualiza_sn        varchar2(1);
   v_fecha_visualiza     date;
   --
begin
    --
    ...
    for v_comctz in cr_comctz(p_comcon_id, p_comprv_id) loop
         --
         select max(comocm.fecha_visualiza)
           into v_fecha_visualiza
           from com_ordenes_compras     comocm
          where comocm.comcon_id = p_comcon_id
            and comocm.comprv_id = p_comprv_id
            and comocm.comoce_estado not in ('NUE','PEN','ANU','RCH');
         --
         if v_fecha_visualiza is null then
            select nvl( max( comocm.fecha_generacion )  ,sysdate)
             ...
             where comocm.comcon_id = p_comcon_id
               ...
               and comocm.comoce_estado not in ('NUE','PEN','ANU','RCH');
               --
               return com_pkg.dia_habil_proximo(v_fecha_generacion, 5 );
         else
               return v_fecha_visualiza;
         end if;
         --
