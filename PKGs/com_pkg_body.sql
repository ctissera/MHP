...
--
   ...
   and COMPDT_ID = p_compdt_id;
   --
   CURSOR cr_comtdr (p_compdt_id number, p_comprv_id number) IS
   select 1
   ...
   --
   -- documentos presentados por proveedor
   CURSOR cr_compdo (p_compdt_id number, p_comprv_id number) IS
   select *
   from com_proveedores_doc
   where comprv_id = p_comprv_id
   and compdt_id = p_compdt_id
...
BEGIN
      ...
      if p_compdt_tipo = 'RUB' then
        OPEN cr_comtdr(p_compdt_id, p_comprv_id);
      ...
      if P_COMPDT_TIPO_DURACION in ('SIN','NOD') then
        return 'VIGENTE';
          ...
          -- obtengo los datos del documento si es que esta presentado
          OPEN cr_compdo(p_compdt_id, p_comprv_id);
...
-- Obtener los documentos Presentados por el Proveedor
  cursor cr_compdo(p_comprv_id number, p_comctt_id number) is
...
--and situacion = 'HAB';
--
--  Tipos de Documentos - Tipo de Proveedor
