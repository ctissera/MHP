...
--   Date and Time:   11:00 Martes Marzo 28, 2017
...
'No hay ayuda disponible para esta página.'
 ...
 ,p_last_upd_yyyymmddhh24miss => '20170328104900'
  );
null;
...
'          and trunc(sysdate) > trunc( com_pkg.DIA_HABIL_PROXIMO( comocm_pkg.OBTN_COMOCM_FECHA_GENERACION(comocm.comcon_id, comocm.comprv_id) ,to_number(:P1_SEGORG_ID), 5) )';
...
'      , ''f?p=&APP_ID.:410:&SESSION.::NO:RP,410:G_APP_ANTERIOR,G_APP_ANTERIOR_PAG,P410_FILTRO_PREDEFINIDO_SEL:&APP_ID.,&APP_PAGE_ID.,OPDE'' url'||unistr('\000a')||
...
s:=s||', ''Calificaciones para Revisar'' descripcion'||unistr('\000a')||
'      , (select to_char(count(1)) cantidad'||unistr('\000a')||
'        FROM com_ordenes_compras comocm'||unistr('\000a')||
'           , com_contrataciones comcon'||unistr('\000a')||
'           , com_contrataciones_tipos comctt'||unistr('\000a')||
...
s:=s||'          and comcon.comctt_id = comctt.id'||unistr('\000a')||
...
s:=s||'_estado <> ''APR'''||unistr('\000a')||
'          and decode(comocm.comoce_estado,''NUE'','''',''ANU'','''',''RCH'','''',''APR'','''',COM_PKG.COMREP_REPLICAR_SN(p_comocm_id => comocm.id,p_origen => ''PRV''))= ''SI'') value'||unistr('\000a')||
...
s:=s||'       AND comptt.comprt_id = (SELECT comprt_id FROM com_proveedores comprv WHERE comprv.ID = :P1_COMPRV_ID)'||unistr('\000a')||
...
'                                         , P_COMPDT_TIPO => COMPDT.';

s:=s||'TIPO'||unistr('\000a')||
...
s:=s||'    where compdt.id = comtdr.compdt_id'||unistr('\000a')||
...
s:=s||'&APP_ID.:510:&SESSION.::NO:RP,510:G_APP_ANTERIOR,G_APP_ANTERIOR_PAG:&APP_ID.,&APP_PAGE_ID.'' url'||unistr('\000a')||
