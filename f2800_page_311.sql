...
--   Date and Time:   14:51 Tuesday March 21, 2017
...
'No hay ayuda disponible para esta página.'
 ...
 ,p_last_upd_yyyymmddhh24miss => '20170315135541'
  );
null;
...
begin
h := null;
h:=h||'Ingrese el Nombre y Apellido del Proveedor en caso de ser una persona física o la Razón Social en caso de ser una persona jurídica.<br><br>'||unistr('\000a')||
'Recuerde, los datos que ingresa tienen carácter de Declaración Jurada por lo cual deben ser exactos, en el caso del Nombre y Apellido tal cual figura en el Documento de Identidad correspondiente y en el caso de la Razón Social tal cual figura en la AFIP.';
...
wwv_flow_api.create_page_item(
  ...
  p_use_cache_before_default=> 'YES',
  p_item_default_type=> 'STATIC_TEXT_WITH_SUBSTITUTIONS',
  p_prompt=>'Nombre y Apellido / Razón Social',
  p_source_type=> 'STATIC',
  p_display_as=> 'NATIVE_TEXT_FIELD',
  p_lov_display_null=> 'NO',
  p_lov_translated=> 'N',
...
'      v_comtpg.ID := COMTPG_ID.nextval;'||unistr('\000a')||
'  '||unistr('\000a')||
'      v_comtpg.COMTPR_ID := :P311_COMTPR_ID;'||unistr('\000a')||
'   ';

p:=p||'   v_comtpg.COMRGP_ID := :P310_COMPRG_ID;'||unistr('\000a')||
...
p:=p||'  v_comtps  COM_TRAMITES_PRV_RES%rowtype;'||unistr('\000a')||
'      '||unistr('\000a')||
'    BEGIN'||unistr('\000a')||
'      --'||unistr('\000a')||
...
p:=p||'omres.OBSERVACION      := null;'||unistr('\000a')||
'      --'||unistr('\000a')||
'      COM_PKG.INS_COMRES(p_comres=> v_comres);'||unistr('\000a')||
'      --      '||unistr('\000a')||
'      '||unistr('\000a')||
...
p:=p||'ID := v_comtps.ID;'||unistr('\000a')||
'      --'||unistr('\000a')||
'    END;'||unistr('\000a')||
'    --'||unistr('\000a')||
'  END IF;'||unistr('\000a')||
