...
'No hay ayuda disponible para esta página.'
 ...
 ,p_last_upd_yyyymmddhh24miss => '20170606113742'
  );
null;
...
s:=s||'sdate) > trunc( com_pkg.DIA_HABIL_PROXIMO( comocm_pkg.OBTN_COMOCM_FECHA_GENERACION(comocm.comcon_id, comocm.comprv_id) ,to_number(:P1_SEGORG_ID), 5) )'||unistr('\000a')||
'  and trunc(sysdate) > trunc( comocm_pkg.OBTN_COMOCM_FECHA_GENERACION(comocm.comcon_id, comocm.comprv_id) )'||unistr('\000a')||
...
a1:=a1||'sdate) > trunc( com_pkg.DIA_HABIL_PROXIMO( comocm_pkg.OBTN_COMOCM_FECHA_GENERACION(comocm.comcon_id, comocm.comprv_id) ,to_number(:P1_SEGORG_ID), 5) )'||unistr('\000a')||
'  and trunc(sysdate) > trunc( comocm_pkg.OBTN_COMOCM_FECHA_GENERACION(comocm.comcon_id, comocm.comprv_id) )'||unistr('\000a')||
'  and comocm.comprv_id = :P410_COMPRV_ID'||unistr('\000a')||
'&P410_WHERE.';
...
'	--:P410_WHERE := :P410_WHERE||'' and comocm.comoce_estado = ''''APR'''' and COMOCM_ENT_PEN_SN2(p_comocm_id => comocm.id) = ''''SI'''''' ;'||unistr('\000a')||
...
p:=p||'ILTRO_PREDEFINIDO_SEL = ''OCEV'' then'||unistr('\000a')||
'	:P410_WHERE := :P410_WHERE||'' and comocm.comoce_estado = ''''APR'''' and COMOCM_PKG.COMOCM_ENT_VENC_SN(p_comocm_id => comocm.id) = ''''SI'''''';'||unistr('\000a')||
'end if;	'||unistr('\000a')||
''||unistr('\000a')||
...
'  :P410_WHERE := :P410_WHERE||'' AND comocm.comoce_esta';

p:=p||'do = ''''''||:P410_COMOCE_ESTADO||'''''''';'||unistr('\000a')||
'end if;  '||unistr('\000a')||
''||unistr('\000a')||
'if nvl(:P410_COMOCM_REPLICA_EST,''0'') <> ''0'' then'||unistr('\000a')||
'  :P410_WHERE := :P410_WHERE||'' AND decode(comocm.comoce_estado,''''RCH'''','''''''',comocm_pkg.comocm_replica_est(p_comocm_id => comocm.id)) = ''''''||:P410_COMOCM_REPLICA_EST||'''''''';'||unistr('\000a')||
'end if;  '||unistr('\000a')||
''||unistr('\000a')||
...
p:=p||'= :P410_WHERE||'' AND exists (select 1 from com_calificaciones comclf where comclf.comocm_id = comocm.id and comclf.situacion = ''''VAL'''' and comclf.calificacion = ''''''||:P410_COMCLF_CALIFICACION||'''''')'';'||unistr('\000a')||
'   else'||unistr('\000a')||
...
p:=p||'CHA_ENTREGA_DESDE IS NOT NULL then'||unistr('\000a')||
'  :P410_WHERE := :P410_WHERE||'' AND comped.fecha_entrega >= to_date(''''''||:P410_FECHA_ENTREGA_DESDE||'''''',''''DD/MM/YYYY'''')'';'||unistr('\000a')||
'end if;'||unistr('\000a')||
'--'||unistr('\000a')||
...
'end if;'||unistr('\000a')||
'--  '||unistr('\000a')||
':P410_WHERE := :P410_WHERE||'' ORDER BY TO_NUMBER(de';

p:=p||'code(comocm.comoce_estado,''''APR'''',''''1'''',decode(COM_PKG.COMREP_REPLICAR_SN(p_comocm_id => comocm.id,p_origen => ''''PRV''''),''''SI'''',''''2'''',''''3''''))) asc, comocm.fecha_generacion desc'';';
